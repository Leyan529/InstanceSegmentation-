...
def encode(matched, anchors):
    ...
    # print("g_wh_max: ", g_wh.max())
    # print("g_wh_min: ", g_wh.min())
    # g_wh    = torch.clamp(g_wh, -20, 20)    
...
class Multi_Loss(nn.Module):
    ...
    def forward(self, predictions, targets, mask_gt, num_crowds):
        ...
        losses['B']     = self.bbox_loss(pos_pred_boxes, pos_offsets) * 1.5 # inf
        losses['C']     = self.ohem_conf_loss(pred_classes, true_classes, positive_bool)
        losses['M']     = self.lincomb_mask_loss(positive_bool, pred_masks, pred_proto, mask_gt, anchor_max_box, anchor_max_index) * 6.125 # nan
    ...
    def ohem_conf_loss(self, pred_classes, true_classes, positive_bool):
        ...
        eps = 1e-5
        mark            = torch.log(torch.sum(torch.exp(batch_conf - batch_conf_max), 1) + eps) + batch_conf_max - batch_conf[:, 0]
        # print("mark: ", mark)
    ...
    def lincomb_mask_loss(positive_bool, pred_masks, pred_proto, mask_gt, anchor_max_box, anchor_max_index):
        ...
        for i in range(pred_masks.size(0)):  
            ...
            # print("mask_p: ", mask_p.shape)
            # print("pos_mask_gt: ", pos_mask_gt.shape)
            mask_loss = F.binary_cross_entropy(mask_p, pos_mask_gt, reduction='none') # CUDA error: device-side assert triggered
            #-----------------------------------------------------#
            #   每个先验框各自计算平均值
            #-----------------------------------------------------#
            eps = 1e-7
            pos_get_csize   = center_size(pos_anchor_box) + eps
            assert not torch.isnan(pos_get_csize).any()
            mask_loss       = mask_loss.sum(dim=(0, 1)) / pos_get_csize[:, 2] / pos_get_csize[:, 3] # 除0 nan
            assert not torch.isnan(mask_loss).any()
            # tensor([    nan, 22.5469, 38.8111,  0.0000,  0.3597, 19.6457,  5.3848,  0.6182,
            #     13.1581,  0.4759, 12.3866,  5.3853,  5.3963, 18.8703,  0.6491,  0.6607,
            #     0.6355,  0.6499,  0.6497,  6.2670,  6.2630,  6.2647,  6.2632],
