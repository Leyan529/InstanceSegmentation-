...
from inst_model.Mask_RCNN.utils.utils import cvtColor, get_classes, preprocess_input, resize_image
from inst_model.Mask_RCNN.utils.anchors import get_anchors
...
class Mask_RCNN(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"        : 'work_dirs/voc_yolact/best_epoch_weights.pth',
    ...
    def generate(self):
        ...
        self.net = MaskRCNN(backbone, num_classes=self.num_classes, use_pre_trained=False, train_mode=False)
    ...
    def detect_image(self, image):
        ...
        with torch.no_grad():
            ...
            #   同時解码并进行非极大抑制
            #---------------------------------------------------------#
            predictions = self.net(image_data)[0]

            predict_boxes = predictions["boxes"].to("cpu").numpy()
            predict_classes = predictions["labels"].to("cpu").numpy()
            predict_scores = predictions["scores"].to("cpu").numpy()
            predict_mask = predictions["masks"].to("cpu").squeeze(1)
           
            # results = self.net(image_data)
            # results = results[0]           
            # results = [results["boxes"], results["labels"], results["scores"], results["masks"]]
        ...
        for i in range(np.shape(class_ids)[0]):
            left, top, right, bottom = np.array(predict_boxes[i, :], np.int32)
            ...
            #   获得这个框的种类并写在图片上
            #---------------------------------------------------------#
            class_name  = self.class_names[class_ids[i]-1]
            text_str    = f'{class_name}: {predict_scores[i]:.2f}'
    ...
    def get_map_out(self, image):
        ...
        with torch.no_grad():
            ...
            results = [masks_arg, masks_sigmoid]
            masks_arg, masks_sigmoid = [x.cpu().numpy() for x in results]

        return boxes, scores, classes, masks_arg, masks_sigmoid
