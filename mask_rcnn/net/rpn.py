...
class RegionProposalNetwork(nn.Module):
    def __init__(self, anchor_generator, head, 
                 ...
                 reg_weights,
                 pre_nms_top_n, post_nms_top_n, nms_thresh, predict):
        super().__init__()
    ...
    def compute_loss(self, objectness, pred_bbox_delta, gt_box, anchor):
        iou = [box_iou(gt_box[i], anchor) for i in range(len(gt_box))]

        objectness_loss = []
        box_loss = []
        for i in range(len(gt_box)):
            label, matched_idx = self.proposal_matcher(iou[i])
            
            pos_idx, neg_idx = self.fg_bg_sampler(label)
            idx = torch.cat((pos_idx, neg_idx))
            regression_target = self.box_coder.encode(gt_box[i][matched_idx[pos_idx]], anchor[pos_idx])            
            objectness_loss.append(F.binary_cross_entropy_with_logits(objectness[i][idx], label[idx]))
            box_loss.append(F.l1_loss(pred_bbox_delta[i][pos_idx], regression_target, reduction='sum') / idx.numel())

        objectness_loss = torch.stack(objectness_loss, dim=0).sum(dim=0) / len(objectness_loss)
        box_loss = torch.stack(box_loss, dim=0).sum(dim=0) / len(box_loss)
    ...
    def forward(self, feature, image_shape, target=None):
        ...
        objectness = objectness.permute(0, 2, 3, 1).flatten(start_dim=1, end_dim=3)
        pred_bbox_delta = pred_bbox_delta.permute(0, 2, 3, 1).flatten(start_dim=1, end_dim=3)
        pred_bbox_delta = pred_bbox_delta.reshape(objectness.shape[0], objectness.shape[1], -1)

        proposal = []
        for i in range(objectness.shape[0]):
            pp = self.create_proposal(anchor, objectness[i].detach(), pred_bbox_delta[i].detach(), image_shape)
            # proposal.append(pp.unsqueeze(0))
            proposal.append(pp)
