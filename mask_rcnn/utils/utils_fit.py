...
def train_one_epoch(model, optimizer, data_loader, device, epoch, args):
    ...
    for i, (images, target) in enumerate(data_loader):
        ...
        target = {k: [item.to(device) for item in v] for k, v in target.items()}
        S = time.time()
    ...
    print("Training loss: {:.6f}".format(t_loss/len(data_loader)))
    # return A / iters
    return t_loss/len(data_loader), A / iters

def eval_one_epoch(model, data_loader, device, epoch, args):
    # for p in optimizer.param_groups:
    #     p["lr"] = args.lr_epoch
    ...
    A = time.time() - A
    print("iter: {:.1f}, total: {:.1f}, model: {:.1f}, evaluate: {:.1f}".format(1000*A/iters, 1000*t_m.avg, 1000*m_m.avg, 1000*b_m.avg))
    print("Evaluate loss: {:.6f}".format(t_loss/len(data_loader)))
    # return A / iters            
    return t_loss/len(data_loader), A / iters
